...
function fool() {
  if (u == 0) {
    return "<p>정방향 : 바보가 정방향으로 나타난다면 당신이 인생의 새로운 단계에 들어서려고 한다는 것이다. \
    ...
    return "<p>역방향 : 바보카드의 역방향은, 만약 당신이 아직 상황을 신중하게 고려하지 않았다면 위험을 감수할 것을 경고한다.\
    벼랑 끝에 서서 떨어질 준비를 하고 싶은 기분이 들지도 모른다. 너는 너의 견해에 속을지도 모른다.\
    어리석음의 지속된다면 지혜의 유령이라는 에머슨(랄프알도에머슨)의 말을 기억하라.\
    현재의 상황은 알지 못하는 것 또는 미래가 가져올 수 있는 것을 두려워하게 만든다.</p>\
    <p>정방향이든 역방향이든 타로의 바보카드는 자연의 반응에 대한 믿음과 정신에 대한 믿음을 지키라고 일깨워준다.\
    만약 여러분이 어떤 결정이나 의심의 순간에 직면하고 있다면, 이 카드는 여러분 자신을 믿고 그것이 아무리 미친 짓일지라도 길을 가라고하는 것을 암시한다.</p>";
  }
}
