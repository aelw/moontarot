...
function fool() {
  if (u == 0) {
    return "<p>이 카드는 어린 시절의 낙천주의와 태평스러움으로 위험을 감수할 필요성을 보여준다. 믿을 수 없는 관점은 문제를 해결하는 방법을 제공할 수 있기 때문에, 당신은 새로운 아이디어에 대한 개방성을 유지할 필요가 있다. 바보는 스트레스, 명확성의 결여, 불확실한 조건들로 표시된 기간을 나타낼 수 있다. 갑작스런 발전은 당신을 무방비 상태로 만들 수 있고 당신이 태도를 회복할 때까지 약간의 불안감을 줄 수 있다.</p><p> 현명하게 과거를 떠나 새로운 일을 시작해야 한다. 잠재의식이 새로운 삶을 시작하도록 안내하고 있기 때문에 전통적이고 구시대적인 방법을 고수하는 것은 잘못된 것일 수도 있다. 때로는 바보가 진짜 여행에 대한 헌신을 보여주기도 하는데, 특히 다른 여행 카드(예: 전차, 운명의 수레바퀴, 세계, 6검, 8완즈 또는 완즈기사)가 주변에 나온다면 더욱 그렇다.</p>";
  } else {
